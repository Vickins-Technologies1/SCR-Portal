...
export default function MaintenanceRequests({ userId, csrfToken, properties }: MaintenanceRequestsProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      // Step 1: Fetch role from cookies
      const userRole = Cookies.get("role");
      console.log("[DEBUG] Fetched role from cookies:", userRole);
      setRole(userRole ?? null);

      // Step 2: Proceed only if role, userId, and csrfToken are available
      if (!userId || !csrfToken) {
        const errorMsg = `Missing userId or csrfToken - userId: ${userId}, csrfToken: ${csrfToken}`;
        console.error("[ERROR] fetchMaintenanceRequests:", errorMsg);
        setError(errorMsg);
        setIsLoading(false);
        return;
      }
      ...
      if (!userRole) {
        console.error("[ERROR] fetchMaintenanceRequests: Role not set");
      ...
      try {
        const queryParams = userRole === "tenant"
          ? `userId=${encodeURIComponent(userId)}&tenantId=${encodeURIComponent(userId)}`
          : `userId=${encodeURIComponent(userId)}`;
        console.log("[DEBUG] Fetching maintenance requests with query:", {
          url: `/api/tenants/maintenance?${queryParams}&page=${currentPage}&limit=${requestsPerPage}`,
          userId,
          role: userRole,
        });

        const res = await fetch(
          `/api/tenants/maintenance?${queryParams}&page=${currentPage}&limit=${requestsPerPage}`,
          {
            method: "GET",
            headers: {
        ...
        if (!res.ok) {
          ...
          throw new Error(`Fetch failed for /api/tenants/maintenance: ${res.status} ${res.statusText} - ${text.slice(0, 50)}`);
        }

        const data = await res.json();
        if (!data.success) {
          console.error("[ERROR] API response unsuccessful:", data);
        ...
        console.log("[INFO] Maintenance requests fetched:", {
          requests: data.data.requests,
          total: data.data.total,
        });
      } catch (err) {
        console.error("[ERROR] Maintenance requests fetch error:", err);
    ...
    fetchData();
  }, [userId, csrfToken, currentPage]); // Removed 'role' from dependencies
  ...
  const handleDeleteRequest = async (requestId: string) => {
    ...
    try {
      ...
      console.error("[ERROR] Delete request error:", err);
        ...
        <>
          ...
          <div className="mt-4 flex items-center justify-between">
            <div className="text-sm text-gray-600">
              ...
              {totalPages * requestsPerPage} requests
            </div>
            <div className="flex gap-2">
              <button
