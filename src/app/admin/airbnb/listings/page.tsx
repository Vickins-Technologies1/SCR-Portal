...
export default function AdminAirbnbListingsPage() {
  ...
  const fetchListings = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/admin/airbnb/listings", {
        method: "GET",
        credentials: "include",
        cache: "no-store",
      });
      ...
      setError(err.message || "Failed to load Airbnb listings.");
    } finally {
      setIsLoading(false);
    }
  }, [status, router]);
                ...
                <table className="w-full">
                  <thead>
                    <tr>
                      ...
                      <th className="py-3 px-4 text-left text-xs font-semibold text-muted-foreground uppercase tracking-wide">Location</th>
                      <th className="py-3 px-4 text-left text-xs font-semibold text-muted-foreground uppercase tracking-wide">Status</th>
                      <th className="py-3 px-4 text-left text-xs font-semibold text-muted-foreground uppercase tracking-wide">Units</th>
                      <th className="py-3 px-4 text-left text-xs font-semibold text-muted-foreground uppercase tracking-wide">Base Rate</th>
