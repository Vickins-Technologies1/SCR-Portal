...
export default function AdminAirbnbPayoutsPage() {
  ...
  const fetchPayouts = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/admin/airbnb/payouts", {
        method: "GET",
        credentials: "include",
        cache: "no-store",
      });
      ...
      const data = await res.json();
      if (!data.success) throw new Error(data.message || "Failed to load payouts");

      setPayouts(data.payouts || []);
    } catch (err: any) {
      setError(err.message || "Failed to load Airbnb payouts.");
    } finally {
      setIsLoading(false);
    }
  }, [status, router]);
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <h1 className="text-xl sm:text-2xl font-semibold text-foreground">Airbnb Payouts</h1>
                  <p className="text-xs sm:text-sm text-muted-foreground">
                    Track payout history and settlement status for Airbnb hosts.
                  </p>
                </div>
              </div>
            </div>
            ...
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="table-shell">
              <div className="table-scroll">
                <table className="w-full">
                  <thead>
                    <tr>
                      <th className="py-3 px-4 text-left text-xs font-semibold text-muted-foreground uppercase tracking-wide">Property</th>
                      <th className="py-3 px-4 text-left text-xs font-semibold text-muted-foreground uppercase tracking-wide">Owner</th>
                      <th className="py-3 px-4 text-left text-xs font-semibold text-muted-foreground uppercase tracking-wide">Period</th>
