...
export default function PropertyOwnerDashboard() {
  ...
  const getPropertyOccupancy = (property: Property) => {
    const totalUnits = property.unitTypes.reduce((sum, ut) => sum + ut.quantity, 0);
    const occupiedUnits = tenants.filter(t => t.propertyId === property._id.toString()).length;
  ...
  // === CHART DATA ===
  const paymentStatusSummary = tenants.reduce((acc, t) => {
    if (!t.leaseEndDate || new Date(t.leaseEndDate) < new Date()) acc.pending++;
    else if (t.paymentStatus === "overdue") acc.overdue++;
    else acc.paid++;
    return acc;
  ...
  const lineData = {
    ...
    datasets: [
      { label: "Rent", data: chartData?.rentPayments || [], borderColor: "#2563eb", backgroundColor: "rgba(37,99,235,0.08)", fill: true, tension: 0.4 },
      { label: "Utility", data: chartData?.utilityPayments || [], borderColor: "#ec4899", backgroundColor: "rgba(236,72,153,0.08)", fill: true, tension: 0.4 },
      { label: "Deposit", data: chartData?.depositPayments || [], borderColor: "#10b981", backgroundColor: "rgba(16,185,129,0.08)", fill: true, tension: 0.4 },
    ],
  };
    ...
    <div className={`min-h-screen bg-gray-50 ${inter.className}`}>
      ...
      <div className="md:ml-64 pt-16 pb-12 px-4 sm:px-6 lg:px-8">
        <main className="max-w-7xl mx-auto">
          ...
          {/* LOADING STATE - WITH NAVBAR & SIDEBAR */}
          {isLoading ? (
            <div className="space-y-8">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-5">
                {[...Array(8)].map((_, i) => (
                  <div
                    key={i}
                    className="bg-white/80 backdrop-blur-sm rounded-2xl h-32 shadow-lg"
                    style={{ animation: `fade 2s ease-in-out ${i * 0.15}s infinite` }}
                  >
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="bg-white/90 backdrop-blur-sm rounded-2xl p-6 h-96 shadow-lg">
                ...
                <div className="bg-white/90 backdrop-blur-sm rounded-2xl p-6 h-96 shadow-lg flex items-center justify-center">
              ...
              {/* Properties Skeleton */}
              <div className="mt-12">
                <div className="h-8 bg-gray-200 rounded-2xl w-64 mb-8" />
                <div className="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-5">
                  {[...Array(8)].map((_, i) => (
                    <div
                      ...
                      style={{ animation: `fadeUp 1.8s ease-out ${i * 0.1}s infinite alternate` }}
                    >
                      <div className="h-1 bg-gradient-to-r from-emerald-400 to-cyan-600" />
            ...
            <>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-5 mb-10">
                {[
                  ...
                  { title: "Vacant", value: stats.totalUnits - stats.occupiedUnits, icon: Home, color: "orange" },
                ].map((s, i) => (
                  <motion.div
                    key={i}
              ...
              {/* Properties Grid – Bigger & More Premium on Large Screens */}
              <section className="mt-12">
                ...
                {properties.length === 0 ? (
                  ...
                  <div className="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-4 gap-6 lg:gap-8">
                    {properties.map((property) => {
                      const { totalUnits, occupiedUnits, vacantUnits, rate, isFullyOccupied, isVacant } = getPropertyOccupancy(property);
                      ...
                      return (
                        ...
                        >
                          ...
                          {/* Occupancy Circle – Slightly Larger on Big Screens */}
                          ...
                          {/* Stats Grid – Bigger Text on Large Screens */}
                          <div className="px-6 lg:px-8 pb-8 pt-4">
                            <div className="grid grid-cols-3 gap-4 lg:gap-6 text-center">
                              <div className="bg-emerald-50/70 rounded-2xl py-4 lg:py-5 border border-emerald-100">
                                <p className="text-xs lg:text-sm font-medium text-emerald-700">Total Units</p>
                              ...
                              <div className="bg-blue-50/70 rounded-2xl py-4 lg:py-5 border border-blue-100">
                                <p className="text-xs lg:text-sm font-medium text-blue-700">Occ.</p>
                            ...
                            <div className="mt-6 text-center">
                              <span
                                className={`inline-block px-5 py-2 rounded-full text-sm lg:text-base font-bold shadow-md ${isFullyOccupied
                              ...
                              >
                                {isFullyOccupied ? "Fully Occ." : isVacant ? "Vacant" : "Partially Occ."}
                              </span>
                            </div>
                          </div>

                          {/* Hover Arrow – Larger on Desktop */}
