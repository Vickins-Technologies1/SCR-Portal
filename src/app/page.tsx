...
export default function Home() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = isSignUp
      ? { name, email, password, phone, confirmPassword, role: "propertyOwner" }
      : { email, password, role: isTenantPortal ? "tenant" : "propertyOwner" };
    ...
    try {
      ...
      console.log("Signin response:", data); // Log response
      if (!data.success) {
        ...
        if (isSignUp) {
          ...
          setTimeout(() => router.push(data.redirect), 100); // Add delay
        } else {
      ...
      <div className="flex flex-col w-full lg:w-1/2 items-center justify-center bg-background px-4 py-10 lg:p-12 relative">
        <div className="w-full max-w-md bg-background shadow-2xl rounded-2xl border border-dark-blue p-6 lg:p-8 space-y-6 transform transition-all duration-300 hover:shadow-3xl">
          ...
          <h1 className="text-3xl lg:text-4xl font-extrabold text-center text-foreground">
            {isSignUp ? "Create Your Account" : isTenantPortal ? "Tenant Portal Login" : "Property Owner Login"}
          </h1>
          {error && (
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            >
              {isLoading ? "Processing..." : isSignUp ? "Sign Up" : isTenantPortal ? "Tenant Login" : "Property Owner Login"}
            </button>
          </form>
          <p className="text-center text-sm text-foreground">
            ...
            <button
              ...
              onClick={() => {
                ...
                if (!isSignUp) setIsTenantPortal(false);
              }}
              className="text-accent font-medium hover:underline transition-colors"
            >
