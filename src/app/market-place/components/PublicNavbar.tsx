...
export default function PublicNavbar() {
  ...
  return (
    <>
      <nav className="fixed top-4 left-0 right-0 z-40">
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex items-center justify-between rounded-full border border-border bg-card backdrop-blur-xl shadow-[0_20px_50px_-35px_rgba(0,0,0,0.35)] px-4 py-2">
            ...
            <div className="hidden lg:flex items-center gap-1 rounded-full bg-muted px-2 py-2">
              {navLinks.map((link) => (
                <Link
                  key={link.label}
                  href={link.href}
                  className="px-4 py-2 text-[10px] font-semibold uppercase tracking-[0.22em] text-muted-foreground transition hover:text-foreground"
                >
                  {link.label}
                </Link>
              ))}
            ...
            <div className="hidden lg:flex items-center gap-2">
              <Link
                href="/"
                className="flex items-center gap-2 rounded-full px-4 py-2 text-[10px] font-semibold uppercase tracking-[0.2em] transition-all text-muted-foreground hover:text-foreground"
              >
                <LogIn size={14} />
                Sign In
              </Link>
              <ThemeToggle variant="icon" className="h-10 w-10" />
              <Link
                href="/sign-up"
            ...
            <button
              ...
              className="lg:hidden p-2 rounded-full bg-muted hover:bg-muted-hover transition"
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-50 w-80 rounded-r-3xl border-r border-border bg-card backdrop-blur-xl shadow-[0_25px_70px_-40px_rgba(0,0,0,0.55)] transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] lg:hidden ${
      ...
      >
        <div className="flex h-full flex-col gap-8 p-6">
            ...
            {navLinks.map((link) => {
              ...
              return (
                <Link
                  key={link.label}
                  href={link.href}
                  className="flex w-full items-center gap-3 rounded-2xl px-4 py-3 text-xs font-semibold tracking-wide text-foreground hover:bg-muted transition"
                  onClick={() => setMobileOpen(false)}
                >
          ...
          <div className="space-y-3">
            <Link
              href="/"
              className="flex w-full items-center justify-between rounded-2xl border border-border px-5 py-3 text-xs font-semibold tracking-wide text-foreground transition"
              onClick={() => setMobileOpen(false)}
            >
              <span>Sign In</span>
              <LogIn size={16} />
            </Link>
            <ThemeToggle variant="icon" className="h-11 w-11" />
            <Link
              href="/sign-up"
