        ...
        <section className="mt-8">
          <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
            <div className="lg:col-span-2 space-y-6 lg:space-y-8">
                ...
                {isAirbnb ? (
                  <div className="mt-6 sm:mt-8 grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div className="rounded-2xl border border-border bg-muted/60 p-5 text-center">
                      <p className="text-xs uppercase tracking-wider text-muted-foreground">Nightly rate</p>
                    ...
                    <div className="rounded-2xl border border-border bg-muted/60 p-5 text-center">
                      <p className="text-xs uppercase tracking-wider text-muted-foreground">Weekend rate</p>
                      <p className="mt-2 text-xl sm:text-2xl font-semibold text-foreground">
              ...
              {isAirbnb ? (
                <div className="surface-card rounded-3xl p-5 sm:p-6 lg:p-8 space-y-6">
                  <div className="flex items-center justify-between">
                    <h2 className="text-xl sm:text-2xl font-semibold text-foreground">Stay details</h2>
                  ...
                  {airbnbListing?.amenities?.length ? (
                    <div>
                      <p className="text-xs uppercase tracking-wider text-muted-foreground mb-3">Amenities</p>
                      <div className="flex flex-wrap gap-2">
                        {airbnbListing.amenities.map((amenity) => (
                          <span
                            key={amenity}
                            className="rounded-full border border-border bg-muted/70 px-3 py-1 text-xs text-muted-foreground"
                          >
                            {amenity}
                          </span>
                        ))}
                  ...
                  {airbnbListing?.houseRules?.length ? (
                    <div>
                      <p className="text-xs uppercase tracking-wider text-muted-foreground mb-3">House rules</p>
                      <ul className="space-y-2 text-sm text-muted-foreground">
                        {airbnbListing.houseRules.map((rule) => (
                          <li key={rule} className="rounded-2xl border border-border bg-muted/60 px-4 py-3">
                            {rule}
                          </li>
                        ))}
                      </ul>
            ...
            <aside className="space-y-6 lg:sticky lg:top-24 lg:h-fit">
                ...
                <div className="rounded-2xl border border-border bg-background/70 p-5">
                  <div className="flex items-center justify-between text-xs uppercase tracking-wider text-muted-foreground mb-3">
                    ...
                    <span className="font-semibold text-foreground">
                      {isAirbnb ? airbnbListing?.occupancyRate ?? 0 : availability?.occupancyRate ?? 0}%
                    </span>
                  </div>
                  <div className="h-3 w-full overflow-hidden rounded-full bg-muted">
                    <div
                      ...
                      style={{
                        width: `${Math.min(100, isAirbnb ? airbnbListing?.occupancyRate ?? 0 : availability?.occupancyRate ?? 0)}%`,
                      }}
                    />
                  </div>
                </div>

                {!isAirbnb && property.facilities?.length ? (
                  <div className="mt-6">
              ...
              <div className="surface-card rounded-3xl p-6">
                ...
                <p className="mt-6 text-sm text-muted-foreground leading-relaxed">
                  {isAirbnb
                    ? "Reach out for booking details, availability, or check-in instructions."
